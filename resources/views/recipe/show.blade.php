...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="flex flex-col space-y-6 lg:space-y-6">
            
            <!-- Nutrition Info (Shows first on mobile, second on desktop) -->
            <div class="bg-white rounded-lg shadow-sm order-1 lg:order-2">
                @livewire('nutrition', ['recipeId' => $recipe->id])
            </div>

            <!-- Social Sharing Card (Shows second on mobile, first on desktop) -->
            <div class="bg-white rounded-lg shadow-sm p-6 order-2 lg:order-1">
                ...
                <div class="flex space-x-2">
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ Request::fullUrl() }}&display=popup" 
                       target="_blank" 
                       class="flex items-center justify-center w-9 h-9 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                    ...
                    <a href="https://twitter.com/intent/tweet?text=I'm cooking {{ $recipe->title }}&url={{ Request::fullUrl() }}" 
                       target="_blank" 
                       class="flex items-center justify-center w-9 h-9 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors"
                       aria-label="Share on Twitter"
                       style="font-size: 14px; font-weight: 900; font-family: Arial, sans-serif;">
                        X
                    </a>
                    <a href="https://www.pinterest.com/pin/create/button/?url={{ Request::fullUrl()}}&media={{ asset('storage/'. $recipe->image) }}&description={{ $recipe->title }}" 
                       target="_blank" 
                       class="flex items-center justify-center w-9 h-9 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"
