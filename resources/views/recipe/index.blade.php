        ...
        <div class="mt-3 px-5 p-md-0">
                ...
                <div class="row row row-cols-1 row-cols-sm-2 row-cols-md-3 ">
                    ...
                    @foreach ($index as $return)
                            ...
                            <div>
                                <a href="{{ route($return->category,$return->slug) }}" rel="nofollow" aria-label="{{$return->title}}"><h5>{{ $return->title }}</h5>
                                </a>({{ $return->total }})
                            </div>
                            ...
                            @if(!$loop->first)
                                ...
                                @else
                                    ...
                                    <h1>{{ $next }}</h1>
                                    <div>
                                        <a href="{{ route($return->category,$return->slug) }}" rel="nofollow" aria-label="{{$return->title}}"><h5>{{ $index[$loop->index]->title }}</h5>
                                        </a>({{ $return->total }})
                                    </div>
